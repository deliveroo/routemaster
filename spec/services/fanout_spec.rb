...
describe Routemaster::Services::Fanout do
  ...
  describe '#run' do
    ...
    context 'with a single subscriber' do
      before { topic.subscribers.add queue }

      it 'passes the event to the queue' do
        topic.push event
        subject.run
        expect(topic.peek).to be_nil
        expect(queue.peek).not_to be_nil
      end
      ...
      it 'preserves order' do
        ...
        topic.push event2
        2.times { subject.run }
        expect(queue.pop.url).to end_with('1')
        expect(queue.pop.url).to end_with('2')
      end
    end
    ...
    context 'with multiple subscribers' do
      ...
      it 'fans out' do
        ...
        expect(queue2.pop).not_to be_nil
      end
    end
  end
end
